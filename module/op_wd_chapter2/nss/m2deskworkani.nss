...
{
    int nRand = Random(4);
    object oDesk = GetNearestObjectByTag("Work_Object");

    if (GetLocalInt(OBJECT_SELF,"NW_L_HEARTBEAT") == 0 && !IsInConversation(OBJECT_SELF))
    {
        if (GetIsObjectValid(oDesk))
        {
            SetFacingPoint(GetPosition(oDesk));
        }
