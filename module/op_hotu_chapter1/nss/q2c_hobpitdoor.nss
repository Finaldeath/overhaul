    ...
    if ((nDone == 0) && (nFoundPCs != 0) && (GetIsObjectValid(oidBestSearcher)))
    {
        ...
        if (GetIsInCombat(oidBestSearcher) == FALSE)
        {
            ...
            if ((nBestSkill +nMod) > nDifficulty)
            {
                ...
                effect vEffect = EffectVisualEffect(VFX_FNF_DISPEL);
                ApplyEffectAtLocation(DURATION_TYPE_INSTANT, vEffect, GetLocation(OBJECT_SELF));
                ...
                SendMessageToPCByStrRef(oidBestSearcher, 84149 );

                //Create destination Door at waypoint "objDoor2_"+sTag
                location lTargetDoor = GetLocation(GetObjectByTag("D2_" + sTag));
                object oDoor2 = CreateObject(OBJECT_TYPE_PLACEABLE,"hidden_wall_door",lTargetDoor,TRUE);
                //Set facing on the second door
                AssignCommand(oDoor2,SetFacing(GetFacing(GetObjectByTag("D2_" + sTag))));
