...
void main ()
{
    ...
    {
        if (GetLocalInt(oPC, "X2_ReapDoor4Warning") != 1)
        {
            SetLocalInt(oPC, "X2_ReapDoor4Warning", 1);
            FloatingTextStrRefOnCreature(83358, oPC, FALSE);
            DelayCommand(10.0, SetLocalInt(oPC, "X2ReapDoor4Warning", 0));
        }
        return;
    }
    ...
    string szAreaWP = PortalGetAnchorName(4, oPC);
    SetCustomToken(4904, GetName(GetArea(GetWaypointByTag(szAreaWP))));
