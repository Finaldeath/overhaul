...
void main()
{
    ...
    if(nUser == 1001) //HEARTBEAT
    {
        if ((GetLocalInt(GetModule(), "X2_ypattack") == 2 || GetLocalInt(GetModule(), "X2_ypattack") == 3) && IsInConversation(OBJECT_SELF) == FALSE)
        {
            ...
            //}
            AssignCommand(OBJECT_SELF, ActionMoveToLocation(GetLocation(GetWaypointByTag("wp_q2aparley"))));
    ...
    {
        SetLocalInt(GetModule(), "X2_cut1_parleyH", 1);
    }
    else if(nUser == 1007) // DEATH
    {
        SetLocalInt(GetModule(), "X2_cut1_parleyD", 1);
    }
