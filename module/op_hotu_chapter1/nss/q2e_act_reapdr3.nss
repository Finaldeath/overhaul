...
void main ()
{
    ...
    {
        if (GetLocalInt(oPC, "X2_ReapDoor3Warning") != 1)
        {
            SetLocalInt(oPC, "X2_ReapDoor3Warning", 1);
            FloatingTextStrRefOnCreature(83358, oPC, FALSE);
            DelayCommand(10.0, SetLocalInt(oPC, "X2ReapDoor3Warning", 0));
        }
        return;
    }
    ...
    string szAreaWP = PortalGetAnchorName(3, oPC);
    SetCustomToken(4903, GetName(GetArea(GetWaypointByTag(szAreaWP))));
