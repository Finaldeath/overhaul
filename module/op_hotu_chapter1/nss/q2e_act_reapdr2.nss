...
void main ()
{
    ...
    {
        if (GetLocalInt(oPC, "X2_ReapDoor2Warning") != 1)
        {
            SetLocalInt(oPC, "X2_ReapDoor2Warning", 1);
            FloatingTextStrRefOnCreature(83358, oPC, FALSE);
            DelayCommand(10.0, SetLocalInt(oPC, "X2ReapDoor2Warning", 0));
        }
        return;
    }
    ...
    string szAreaWP = PortalGetAnchorName(2, oPC);
    SetCustomToken(4902, GetName(GetArea(GetWaypointByTag(szAreaWP))));
