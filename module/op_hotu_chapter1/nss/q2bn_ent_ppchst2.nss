...
void main()
{
    if (GetLocalInt(GetModule(), "X2_q2bn_chestspawned") == 1)
        return;
    if (GetLocalInt(GetModule(), "X2_q2bn_chest2taken") == 1)
        return;
    PlaySound("as_cv_shopmetal2");
    SetLocalInt(GetModule(),"X2_q2bn_chestspawned", 1);
    SetLocalInt(OBJECT_SELF, "nDespawnActive", 1);
    location lTarget = GetLocation(GetWaypointByTag("wp_q2bn_ppchest2"));
    effect eVis = EffectVisualEffect(VFX_IMP_BLIND_DEAF_M);
    ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, lTarget);
    CreateObject(OBJECT_TYPE_PLACEABLE, "q2bn_ppchest2", lTarget);
