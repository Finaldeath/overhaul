...
void main()
{
    ...
    if(nUser == 1001) //HEARTBEAT
    {
        if ((GetLocalInt(GetModule(), "X2_ypattack") == 2 || GetLocalInt(GetModule(), "X2_ypattack") == 3) && IsInConversation(OBJECT_SELF) == FALSE)
        {
            ...
            AssignCommand(OBJECT_SELF, ActionMoveToLocation(GetLocation(GetWaypointByTag("wp_q2alafontaine"))));
    ...
    {
        SetLocalInt(GetModule(), "X2_cut1_lafontaineH", 1);
    }
    else if(nUser == 1007) // DEATH
    {
        SetLocalInt(GetModule(), "X2_cut1_lafontaineD", 1);
    }
