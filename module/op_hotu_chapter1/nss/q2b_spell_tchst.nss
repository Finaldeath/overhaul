...
void main()
{
    ...
    {
        object oWaypoint = GetWaypointByTag("wp_q2b_tchest_treas");
        //The chest objects
        ...
        if (GetLocalInt(GetModule(), "X2_Q2ALev1NDispelReward") == 1)
            return;

        SetLocalInt(GetModule(), "X2_Q2ALev1NDispelReward", 1);
        if (GetPCTotalLevel(oPC) < 15)
        {
            Reward_2daXP(oPC, 13, FALSE); //1500 xp reward if PC is less than 15th level
        }
        else
        {
            Reward_2daXP(oPC, XP_MEDIUM_HIGH, FALSE); //200 xp reward if PC is 15th level or higher

        }
        AssignCommand(GetLastSpellCaster(), PlaySound("as_cv_bell2"));
        CreateObject(OBJECT_TYPE_PLACEABLE, "q2b_treasure", GetLocation(oWaypoint));
    }
}
