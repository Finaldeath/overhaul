...
void main()
{
    ...
    if(nUser == 1001) //HEARTBEAT
    {
        if (GetLocalInt(GetModule(), "X2_ypattack") == 2 || GetLocalInt(GetModule(), "X2_ypattack") == 3)
        {
            ...
            AssignCommand(OBJECT_SELF, ActionMoveToLocation(GetLocation(GetWaypointByTag("wp_q2asabine"))));
    ...
    else if(nUser == 1003) // END OF COMBAT
    {
        if (Random(4) == 1)
        {
            switch (Random(3) + 1)
            {
                case 1: PlaySpeakSoundByStrRef(84020);
                        break;
                case 2: PlaySpeakSoundByStrRef(84021);
                        break;
                case 3: PlaySpeakSoundByStrRef(84022);
                        break;
            }
        }
    }
    ...
    {
        SetLocalInt(GetModule(), "X2_cut1_sabineH", 1);
    }
    else if(nUser == 1007) // DEATH
    {
        SetLocalInt(GetModule(), "X2_cut1_sabineD", 1);
    }
    ...
    else if(nUser == 1011) // On Spell Cast At
    {
        if (GetLastSpell() == SPELL_CURE_CRITICAL_WOUNDS || GetLastSpell() == SPELL_CURE_LIGHT_WOUNDS || GetLastSpell() == SPELL_CURE_MINOR_WOUNDS || GetLastSpell() == SPELL_CURE_MODERATE_WOUNDS || GetLastSpell() == SPELL_CURE_SERIOUS_WOUNDS || GetLastSpell() == SPELL_HEAL || GetLastSpell() == SPELL_LESSER_RESTORATION || GetLastSpell() == SPELL_RESTORATION || GetLastSpell() == SPELL_GREATER_RESTORATION || GetLastSpell() == SPELL_RAISE_DEAD || GetLastSpell() == SPELL_RESURRECTION)
        {
            switch (Random(3) + 1)
            {
                case 1: PlaySpeakSoundByStrRef(84023);
                        break;
                case 2: PlaySpeakSoundByStrRef(84024);
                        break;
                case 3: PlaySpeakSoundByStrRef(84025);
                        break;
            }
