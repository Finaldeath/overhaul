...
// * if not runnning normal or better Ai then exit for performance reasons
    ...
    if(GetLocalInt(OBJECT_SELF, "Q2_IGNORE_DEFAULT_AI") == 1)
    {
        if(GetIsPC(oPercep))
        {
            int nDoOnce = GetLocalInt(OBJECT_SELF, "DO_FIRST_ATTACK_ONCE");
            if(nDoOnce != 1)
            {
                SetLocalInt(OBJECT_SELF, "DO_FIRST_ATTACK_ONCE", 1);
                ActionCastSpellAtObject(SPELL_HOLD_PERSON, oPercep, METAMAGIC_ANY, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
                ActionCastSpellAtObject(SPELL_HAMMER_OF_THE_GODS, oPercep);
                ActionDoCommand(SetLocalInt(OBJECT_SELF, "Q2_IGNORE_DEFAULT_AI", 0));
                ActionDoCommand(SignalEvent(OBJECT_SELF, EventUserDefined(110)));
                return;
            }
            int nDoSecondAttack = GetLocalInt(OBJECT_SELF, "SECOND_ATTACK");
            if(nDoSecondAttack == 0)
                return;
            nDoOnce = GetLocalInt(OBJECT_SELF, "DO_SECOND_ATTACK_ONCE");
            if(nDoOnce == 0)
            {
                SetLocalInt(OBJECT_SELF, "DO_SECOND_ATTACK_ONCE", 1);
                ActionCastSpellAtObject(SPELL_DIVINE_POWER, OBJECT_SELF, METAMAGIC_ANY, FALSE, 0, PROJECTILE_PATH_TYPE_DEFAULT, TRUE);
                ActionAttack(oPercep);
                SetLocalInt(OBJECT_SELF, "Q2_IGNORE_DEFAULT_AI", 0);
                //ActionDoCommand(SignalEvent(OBJECT_SELF, EventUserDefined(110)));
                return;
            }
