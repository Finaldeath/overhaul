    ...
    // * BK FEB 2003 Only fight if you can see them. DO NOT RELY ON HEARING FOR ENEMY DETECTION
        ...
        if(!GetHasEffect(EFFECT_TYPE_SLEEP)) {
            if(GetBehaviorState(NW_FLAG_BEHAVIOR_SPECIAL)) {
                ...
                object oBarrel = GetObjectByTag("Q5_SECONDBARREL");
                if (GetIsObjectValid(oBarrel) == FALSE)
                    oBarrel = GetNearestObjectByTag("Q5_GREASE_BARREL");
                if(oBarrel != OBJECT_INVALID && GetDistanceBetween(OBJECT_SELF, oBarrel) <= 30.0)
                {
                    int nAllow = GetLocalInt(GetArea(OBJECT_SELF), "Q5A_CAN_BLOW_BARRELS");
                    if(nAllow == 1) // only attack the barrel if player is in the barrel area
