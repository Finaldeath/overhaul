...
// **Should now be a parameter that allows you to keep associates if you want to**
...
void CutSpeakStringByStrRef(int nCutscene, float fDelay, object oSpeaker, init nString, int iShift = TRUE)
...
void CutPlayVoiceChat(int nCutscene, float fDelay, object oPC, int nVoiceChatID, object oTarget, int iShift = TRUE)
*/
float GetShift(object oObject, int iShift)
{
        float fShift;
...
// Used for bubble text type speaking. Uses the Str Ref table instead of a string
/* Example: CutSpeakString(0.0, GetObjectByTag("drow_priest"), 40277); would have the object drow_priest speak the
   line "I like green eggs and ham." after no delay.
*/
void CallSpeakStringByStrRef(int nCutscene, object oSpeaker, int nString)
{
    if (GetLocalInt(GetModule(), "nCut" + IntToString(nCutscene) + "Aborted") != 1)
    {
        if(GetIsPC(oSpeaker))
        {
            if(nCutscene == GetLocalInt(oSpeaker, "nCutsceneNumber"))
            {
                AssignCommand(oSpeaker, SpeakStringByStrRef(nString));
            }
        }
        else
        {
            AssignCommand(oSpeaker, SpeakStringByStrRef(nString));
            AssignCommand(oSpeaker, PlaySoundByStrRef(nString, FALSE));
        }
    }
}

void CutSpeakStringByStrRef(int nCutscene, float fDelay, object oSpeaker, int nString, int iShift = TRUE)
{
        DelayCommand(fDelay, DelayCommand(GetShift(oSpeaker, iShift), CallSpeakStringByStrRef(nCutscene, oSpeaker, nString)));
}
...
void CallPlayVoiceChat(int nCutscene, object oPC, int nVoiceChatID, object oTarget)
{
    if(nCutscene == GetLocalInt(oPC, "nCutsceneNumber"))
    {
        PlayVoiceChat(nVoiceChatID, oTarget);
    }
}
void CutPlayVoiceChat(int nCutscene, float fDelay, object oPC, int nVoiceChatID, object oTarget)
{
    DelayCommand(fDelay, CallPlayVoiceChat(nCutscene, oPC, nVoiceChatID, oTarget));
}
