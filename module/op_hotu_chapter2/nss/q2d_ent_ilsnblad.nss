//::///////////////////////////////////////////////
//:: Illusionary Blade Barrier: On Enter
//:: q2d_ent_ilsnblad.nss
...
void main()
{
    ...
    if (GetIsPC(oTarget) == FALSE)
        return;
    //if the PC has already disbelieved this barrier
    if (CheckDisBeliever(oTarget) == TRUE)
        return;
...
void DisbelieveWall(object oTarget)
{
    ...
    if (GetLocalInt(OBJECT_SELF, "nMessageDisplayed") != 1)
    {
        SetLocalInt(OBJECT_SELF, "nMessageDisplayed", 1);
        FloatingTextStrRefOnCreature(85721, oTarget, FALSE);//"You recognize this wall of swords to be an illusion."
    }
    ClearAOE(oTarget);
    if (GetTag(OBJECT_SELF) != "vfx_q2d4_bladefake")
    {
