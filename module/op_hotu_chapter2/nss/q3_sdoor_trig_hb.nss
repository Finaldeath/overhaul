...
void main()
{
       ...
       )
    {
       ...
       if ((nBestSkill +nMod > nDiffaculty))
       {
            ...
            oidDoor = CreateObject(OBJECT_TYPE_PLACEABLE,"q3_secret_door1",locLoc,TRUE);
            int nLen = GetStringLength(sTag);
            string sNewTag = GetStringLeft(sTag, nLen - 1);
            object oOtherDoor;
            if(GetStringRight(sTag, 1) == "a")
            {
                oOtherDoor = GetObjectByTag(sTag + "b");
                SetLocalString( oidDoor, "Destination" , "q3d_wp_secret_door2" );
            }
            else
            {
                oOtherDoor = GetObjectByTag(sTag + "a");
                SetLocalString( oidDoor, "Destination" , "q3d_wp_secret_door1" );
            }
            object oDest = GetNearestObjectByTag(GetLocalString(oidDoor, "Destination"));
            SignalEvent(oOtherDoor, EventUserDefined(101));
