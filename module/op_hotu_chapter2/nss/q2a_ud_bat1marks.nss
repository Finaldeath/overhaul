   ...
   int nUser = GetUserDefinedEventNumber();
    ...
    if(nUser == UNIT_MOVE_TO_TARGET )
    {
        ...
        else
        {
            ...
            DelayCommand(3.0, SignalEvent(OBJECT_SELF, EventUserDefined(UNIT_ORDER_PATROL_CURRENT_LOCATION)));
        }
    }
    ...
    else if (nUser == UNIT_ORDER_MAKE_ME_LEADER)
    {
        ...
        SetLocalInt(OBJECT_SELF, "nLeader", 1);
    ...
    else if (nUser == LEADER_ORDER_CHECK_FOR_ENEMIES)
    {
        ...
        if (GetObjectSeen(oEnemy, OBJECT_SELF) == TRUE || GetObjectHeard(oEnemy, OBJECT_SELF) == TRUE)
        {
        ...
        //Only patrol if not ordered elsewhere.
            ...
            if (Random(2) == 1)
            {
                ...
                if (GetIsObjectValid(oGate) == TRUE)
                    szTarget = "wp_bat1patrol1";
                else
                    szTarget = "wp_bat1patrol2";
                object oTarget = GetNearestObjectByTag(szTarget);
                ActionMoveToObject(oTarget, FALSE);
                DelayCommand(3.0, ClearAllActions());
                DelayCommand(3.5, DetermineCombatRound());
                DelayCommand(6.0, SignalEvent(OBJECT_SELF, EventUserDefined(UNIT_ORDER_PATROL_TOWER)));
            }
            else
            {
                ClearAllActions();
