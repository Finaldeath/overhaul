...
void main()
{
    ...
    //Apply Ghost VFX.
    effect eSmoke = SupernaturalEffect(EffectVisualEffect(VFX_DUR_GHOST_SMOKE_2));
    effect eGhost = SupernaturalEffect(EffectVisualEffect(VFX_DUR_GHOST_TRANSPARENT));
    ApplyEffectToObject(DURATION_TYPE_PERMANENT, eSmoke, OBJECT_SELF);
    ApplyEffectToObject(DURATION_TYPE_PERMANENT, eGhost, OBJECT_SELF);

    object oSelf = OBJECT_SELF;

    DelayCommand(0.5, AdjustAlignment(OBJECT_SELF, ALIGNMENT_CHAOTIC, 100));
    DelayCommand(0.5, AdjustAlignment(OBJECT_SELF, ALIGNMENT_EVIL, 100));
}
