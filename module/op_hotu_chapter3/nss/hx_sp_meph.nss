...
void main()
{
    ...
    */
    ...
    // *
     DelayCommand(7.0, SetSpawnInCondition(NW_FLAG_HEARTBEAT_EVENT));
    ...
    if(GetMephCount() > 0)
    {
        ...
        int iMeph = GetMephDamage();

        // Set Meph to his last damage. Then have him fly in.
        // We have to check the damage being done so that it won't fire the next event
        // in the damage script.
        // First return, damage can't be equal to or more than half his health.
        // Last return, damage can't be equal to or more than 1/6 his health.
        if(GetMephCount() == 1)
        {
            if((iMax - iMeph) <= (iMax / 2))
            {
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage((iMax / 2) - 20), OBJECT_SELF);
            }
            else
            {
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(iMeph), OBJECT_SELF);
            }
        }
        else if(GetMephCount() == 2)
        {
            if((iMax - iMeph) <= (iMax / 6))
            {
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(((iMax / 6) * 5) - 10), OBJECT_SELF);
            }
            else
            {
                ApplyEffectToObject(DURATION_TYPE_INSTANT, EffectDamage(iMeph), OBJECT_SELF);
        ...
        DelayCommand(1.0, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(VFX_FNF_SCREEN_SHAKE), GetLocation(OBJECT_SELF)));
        DelayCommand(1.0, ApplyEffectAtLocation(DURATION_TYPE_INSTANT, EffectVisualEffect(460), GetLocation(OBJECT_SELF)));
        DelayCommand(0.8, PlaySound("fs_flapfeather"));

        DelayCommand(1.0, DayToNight(GetFirstPC(), 1.5));
        DelayCommand(1.5, SetMusic(OBJECT_SELF));
        if(GetMephCount() != 0)
        {
            DelayCommand(4.0, ActionAttack(GetFirstPC()));
        }
    }
}
