...
void main()
{
   ...
   DelayCommand(1.0, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, EffectPolymorph(POLYMORPH_TYPE_WOLF, TRUE), GetLastSpeaker(), 10.0));
   DelayCommand(1.0, DestroyObject(oBlock));
   //DelayCommand(61.0, MakeBlock(GetLocalLocation(GetModule(), "HX_WOLF_BLOCK")));
   SetLocalInt(GetModule(), "HX_SEEN_WOLF_FORM", TRUE);
}
