...
void main()
{
    ...
    if(nEvent == 1001) //HEARTBEAT
    {
         int iQuest = GetLocalInt(GetModule(), "bScrivenerQuestComplete");
         int iCutDone = GetLocalInt(GetModule(), "HX_SCRIV_CUT_DONE");

         if(iQuest == TRUE && iCutDone == TRUE)
         {
            effect eTeleport = EffectVisualEffect(VFX_IMP_SUPER_HEROISM);
            location lScrivener = GetLocation(OBJECT_SELF);
            ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eTeleport, lScrivener);
            DelayCommand(0.2, AssignCommand(OBJECT_SELF, ClearAllActions()));
            SetIsDestroyable(TRUE, FALSE, FALSE);
            DestroyObject(OBJECT_SELF, 0.4);

/*            location lLoc = GetLocation(GetObjectByTag("WP_Hellbreath_2"));

            if(GetDistanceBetweenLocations(GetLocation(OBJECT_SELF), lLoc) < 2.0)
            {
