...
void main()
{
    ...
    // fail-safe to spawn skeletons when saving and loading a game after killing a big skeleton
    if(GetLocalInt(OBJECT_SELF, "SIGNAL_PENDING") == 1)
    {
        object oGarg = GetObjectByTag("q4a_garg_skele4x");
        SignalEvent(oGarg, EventUserDefined(4444));
        SetLocalInt(OBJECT_SELF, "SIGNAL_PENDING", 0);
    }
