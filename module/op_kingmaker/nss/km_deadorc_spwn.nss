...
void main()
{
    ...
    */
    ...
    // Create the ghost and kill your self.
    object oSelf = OBJECT_SELF;
    object oGhost = CreateObject(OBJECT_TYPE_CREATURE, "km_orcghost", GetLocation(oSelf));

    effect eDeath = EffectDeath();
    effect eDifferent = EffectVisualEffect(VFX_DUR_GLOW_LIGHT_RED);

    SetLootable(oSelf, TRUE);
    SetIsDestroyable(FALSE, FALSE, TRUE);

    SetLocalObject(oSelf, "GHOST", oGhost);

    DelayCommand(1.0f, AssignCommand(oGhost, ActionRandomWalk()));

    DelayCommand(1.0f, ApplyEffectToObject(DURATION_TYPE_PERMANENT, eDeath, oSelf));
    DelayCommand(1.0f, ApplyEffectToObject(DURATION_TYPE_PERMANENT, eDifferent, oGhost));
