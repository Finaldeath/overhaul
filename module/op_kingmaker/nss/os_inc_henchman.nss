...
//    object oFormerHench = GetAss*ociate(ASSOCIATE_TYPE_HENCHMAN, oPC, 1);
    ...
    if ( (nCountHenchmen  >= nMaxHenchmen -1) && bAdd == TRUE)
    {
        ...
        if ( GetIsObjectValid(oAssociate) )
        {
            ...
            if (GetLocalInt(oAssociate, "X2_JUST_A_FOLLOWER") == FALSE)
            {
                ...
                if (sTag == "os_hen_kai")
                {
                    ...
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_INVALID, 0, 3);
                }
                else
                if (sTag == "os_hen_cal")
                {
                // * leveling up as a fighter would have to be exposed
                    nLevel = AdjustXP2Levels(nLevel);
                    LevelHenchmanUpTo(oAssociate, nLevel,CLASS_TYPE_INVALID , 0, 4);
                }
                else
                if (sTag == "os_hen_trp")
                {   //SpawnScriptDebugger();
                    nLevel = AdjustXP2Levels(nLevel);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_INVALID, 0, 8);
                }
                else
                if (sTag == "os_hen_jab")
                {
                    nLevel = AdjustXP2Levels(nLevel);
                    LevelHenchmanUpTo(oAssociate, nLevel, CLASS_TYPE_INVALID, 0, 9);
                }
                else
                {
