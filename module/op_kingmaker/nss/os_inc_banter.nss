...
void AttemptInterjectionOrPopup(object oSelf, string sTag, object oTrigger,
    int nOverrideModNum=0, object oOverrideBanter=OBJECT_INVALID);

// * Have the proper plot variables been set?
int AttemptToShowInfo(int nTriggerType, int nId, int bDestroyAnyways = FALSE,
...
void FireInterjection(int nType, int nModNumber, object oMaster, object oHench,
    int nDestroyTriggerAnyways = FALSE, string sConvFile="");
...
void WrapInterjection(int nType, int nId, object oMaster, object oHench, string sTag,
    int nDestroyTriggerAnyways = FALSE, string sConvFile="");
// * Wrapper function to streamline that party banter system
int TryBanterWith(string sTag, int nBanter);
// * Calibast or Kaidala romance dialog
...
int BanterReady(string sTag, string sTag2="", string sTag3="")
{
    ...
    if (GetIsObjectValid(oHench3) == TRUE)
    {
        ...
        {
           return FALSE;   // * because one of the participants is not available,
                           //entire interjection needs
                           // * to be dropped.
        }
    }
    // * Are we expecting another participant?
   ...
   // AssignCommand(oHench1, ClearAllActions()); DOn't clear my actions,
...
// * Making code reusable. Call this at the highest level to fire the appropriate
//interjection
void WrapInterjection(int nType, int nId, object oMaster, object oHench,
    string sTag, int nDestroyTriggerAnyways = FALSE, string sConvFile="")
{
    int nStart = 0;
    int nCount = 12;
       ...
       // * these henchmen do not have these lines
        if (bOriginal == TRUE)
        {
            ...
            if (GetIsObjectValid(oDeekin) == TRUE
                && GetMaster(oDeekin) == GetMaster(oHench))
            {    //   AssignCommand(oHench, SpeakString("Give this to Deekin"));
                AttemptInterjectionOrPopup
                    (OBJECT_SELF, "os_inter_nr", GetMaster(oHench), nCode - 3000, oDeekin);
            }
            bReturn = FALSE;
        }
        break;
    ...
    int nOverrideModNum=0, object oOverrideBanter=OBJECT_INVALID)
{
    ...
    if (GetIsObjectValid(oHench) == FALSE)
    {
        ...
        if (sTag == "os_oneliner_nr" || sTag == "os_inter_nr")
        {
    ...
    // * leave the tag as os_inter_nr
    // * and it will grab a random henchman
    // **********************
    if (FindSubString(sTag, "os_inter_nr") >-1)
    {
        if (MOD_EVENT_NUMBER >= 100)
            return;

        WrapInterjection(TRIGGER_INTERJECTION_NONRANDOM, MOD_EVENT_NUMBER,
            oMaster, oHench, sTag, FALSE, sConvFile);
    }
    else
    // **********************
    // * Interjection
    ...
    if (FindSubString(sTag, "os_inter_r") >-1)
    {
        ...
        // * blanked the text.
        WrapInterjection(TRIGGER_INTERJECTION_RANDOM, MOD_EVENT_NUMBER, oMaster, oHench,
            sTag, FALSE, sConvFile);
    }
    // **********************
    // * Interjection
    // * Party, Random
    ...
    if (FindSubString(sTag, "os_party_r") >-1)
    {
        // * if henchman can't speak, don't do this.
        // * unlike other triggers, don't destroy in this case
        // * because it hurts nothing for it to try to fire later.
...
void AttemptRomanceDialog(object oPC, int nLimit)
{
            /* In Chapter2
            iKaidalaStage:
            This variable increases by 1 for each background/romance story Kaidala
            initiates.
            It goes up to 4 in Chapter 2.
            AT THE END OF CHAPTER 2, IT AUTOMATICALLY GETS SET TO 4.
            It then jumps to 5 with the consummate romance trigger at the start of
            Chapter 3, then goes to 6 with the
            romance dialog Kaidala inits in Chapter 3.
            WHEN THE PARTY PASSES THROUGH THE DOOR BACK TO WATERDEEP IN CHAPTER 3
            this variable has to get set to 99.
            */

            /* Logic
                - I rested.
                - Is the plot completion variable for Chapter 2 further along than
                the romance variable for
                 my romance partner
                    - Init conversation
            ...
            */
            ...
            // * Pick appropriate henchman
                ...
                int bHasCalibast = FALSE;
                int bHasKaidala = FALSE;


                object oCalibast = GetObjectByTag("os_hen_cal");
                object oKaidala = GetObjectByTag("os_hen_kai");

                object oHenchToSpeak = OBJECT_INVALID;

                if (GetIsObjectValid(oCalibast) == TRUE)
                {
                    if (GetMaster(oCalibast) == oPC)
                    {
                        bHasCalibast = TRUE;
                    }
                }
                if (GetIsObjectValid(oKaidala) == TRUE)
                {
                    if (GetMaster(oKaidala) == oPC)
                    {
                        bHasKaidala = TRUE;
                    }
                }
                ...
                if (bPlayerGender ==  GENDER_MALE)
                {
                    ...
                    if (bHasKaidala == TRUE &&
                        GetLocalInt(GetModule(), "iKaidalaStage") < nLimit)
                    {
                        oHenchToSpeak = oKaidala;
                    }
                    else
                    if (bHasCalibast == TRUE &&
                        GetLocalInt(GetModule(), "iCalibastStage") < nLimit)
                    {
                        oHenchToSpeak = oCalibast;
                    }
                }
                else
...
int GetPlotProgress()
{
    ...
    //Isle of the Maker
    if (GetLocalInt(GetModule(), "x2_plot_golem_in") == 1)
    {
        nPlots++;
    }
