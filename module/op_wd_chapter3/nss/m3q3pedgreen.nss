void main()
{
    if((GetTag(GetInventoryDisturbItem()) == "M3Q3DGEM012")
        && GetLocalInt(OBJECT_SELF,"M3Q3GreenLight") == 0)
    {
     ...
     ApplyEffectAtLocation(DURATION_TYPE_INSTANT, eVis, GetLocation(OBJECT_SELF),3.0);
      ...
      location lWay = GetLocation(GetObjectByTag("WP_M3Q3D_PEDGREE"));
     CreateObject(OBJECT_TYPE_PLACEABLE,"M3Q3D_SPARKGREEN",lWay);
     DestroyObject(GetInventoryDisturbItem());
     SetLocalInt(OBJECT_SELF,"M3Q3GreenLight",1);
     SignalEvent(GetObjectByTag("M3Q3PedStar"),EventUserDefined(300));
    }
    else if (GetInventoryDisturbType() == INVENTORY_DISTURB_TYPE_ADDED &&
        GetTag(GetInventoryDisturbItem()) != ("M3Q3DGEM012")
            && GetTag(GetInventoryDisturbItem()) !=("M3Q3D_PED_GREEN"))
    {
