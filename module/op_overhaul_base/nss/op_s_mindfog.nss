...
void main()
{
    if (GetCurrentlyRunningEvent() == EVENT_SCRIPT_AREAOFEFFECT_ON_OBJECT_ENTER)
    {
        ...
        if (GetSpellTargetValid(oTarget, oCaster, SPELL_TARGET_STANDARDHOSTILE))
        {
            ...
            {
                SendMessageToPC(oTarget, "*You previously saved and so are immune to this mind fog*");
            }
        }
    }
    ...
    else
    {
        ...
        ApplySpellEffectAtLocation(DURATION_TYPE_TEMPORARY, eAOE, lTarget, GetDuration(1 + (nCasterLevel / 2), ROUNDS));
    }
}
