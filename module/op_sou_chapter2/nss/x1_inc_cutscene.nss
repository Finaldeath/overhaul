...
// Used for bubble text type speaking (TalkTable variant. For use with official
// content that will require translation).
// Example: CutSpeakStrinByStrRef(0.0, GetObjectByTag("drow_priest"), 41060);
// would have the object drow_priest speak the string associated with line 41060
// in the talk table, after no delay.
//
...
//
void CutActionMoveToObject(int nCutscene, float fDelay, object oPC, object oTarget, int iRun, int iShift = TRUE, float fDistance = 1.0);
...
//
void CutFadeToBlack(int nCutscene, float fDelay, object oObject, int iShift = TRUE, float fSpeed = FADE_SPEED_MEDIUM);

// Used to fade in.
// EXAMPLE: CallFadeFromBlack(35.0, oPC); would Fade the screen
// in on the PC, after a delay of 35 seconds.
//
void CutFadeFromBlack(int nCutscene, float fDelay, object oObject, int iShift = TRUE, float fSpeed = FADE_SPEED_MEDIUM);
...
/* EXAMPLE: CutSetCutsceneMode(10.0, oPC, TRUE); would turn cutscene mode
   ...
   Last parameter is whether or not PCs associates should be destroyed or not
*/
...
// Private function used to implement CutSpeakStringByStrRef
...
// Private function used to implement the same function with the "Cut" prefix in
// place of the "Call" prefix.
void CallActionMoveToObject(int nCutscene, object oPC, object oTarget, int iRun, float fDistance = 1.0)
{
    if(GetIsPC(oPC))
    {
        if(nCutscene == GetLocalInt(oPC, "nCutsceneNumber"))
        {
            ...
            {
                SetCommandable(TRUE);
                AssignCommand(oPC, ActionMoveToObject(oTarget, iRun, fDistance));
...
void CutActionMoveToObject(int nCutscene, float fDelay, object oPC, object oTarget, int iRun, int iShift = TRUE, float fDistance = 1.0)
{
    DelayCommand(fDelay, DelayCommand(GetShift(oPC, iShift), CallActionMoveToObject(nCutscene, oPC, oTarget, iRun, fDistance)));
}
...
// Used to fade out and back in at medium speed, with a black screen section of
// 2 seconds in between.
// EXAMPLE: CallFadeOutAndIn(35.0, oPC); would Fade the screen
// out and back in on the PC, after a delay of 35 seconds.
//
...
// Private function used to implement the same function with the "Cut" prefix in
// place of the "Call" prefix.
void CallFadeToBlack(int nCutscene, object oObject, float fSpeed = FADE_SPEED_MEDIUM)
{
    if(nCutscene == GetLocalInt(oObject, "nCutsceneNumber"))
    {
        FadeToBlack(oObject, fSpeed);
    }
}
...
void CutFadeToBlack(int nCutscene, float fDelay, object oObject, int iShift = TRUE, float fSpeed = FADE_SPEED_MEDIUM)
{
    DelayCommand(fDelay, DelayCommand(GetShift(oObject, iShift), CallFadeToBlack(nCutscene, oObject, fSpeed)));
}


// Private function used to implement the same function with the "Cut" prefix in
// place of the "Call" prefix.
void CallFadeFromBlack(int nCutscene, object oObject, float fSpeed = FADE_SPEED_MEDIUM)
{
    if(nCutscene == GetLocalInt(oObject, "nCutsceneNumber"))
    {
        FadeFromBlack(oObject, fSpeed);
    }
}
...
void CutFadeFromBlack(int nCutscene, float fDelay, object oObject, int iShift = TRUE, float fSpeed = FADE_SPEED_MEDIUM)
{
    DelayCommand(fDelay, DelayCommand(GetShift(oObject, iShift), CallFadeFromBlack(nCutscene, oObject, fSpeed)));
}
