...
void main()
{
    ...
    {
        FloatingTextStringOnCreature("You must be in possession of a focus crystal for Master Drogan's ring to work.", oPC, FALSE);
    }
    else
    {
        ...
        if (GetLocalInt(GetModule(), "X1_CUT1RUNNING") == 0)
        {
            ...
            {
                FloatingTextStringOnCreature("Master Drogan has warned against using this ring except in emergencies!", oPC, FALSE);
        ...
        else
        {
            ...
            {
                //SPECIAL CASE - Won't work in this area
                if (GetTag(GetArea(oPC)) == "q2_ElvenCryptUpperLevel")
                {
                    SendMessageToPC(oPC, "Some kind of magical force is blocking the power of the ring.");
            ...
            {
                SendMessageToPC(oPC, "The ring may not be used while you are in combat.");
            }
...
void CheckCrystalCount(object oPC)
{
    ...
    if (GetIsObjectValid(oCrystal) == FALSE)
        FloatingTextStringOnCreature("The ring has consumed your last focus crystal!", oPC, FALSE);
}
